...
# Exporting data from regional projects

log_dir = os.path.join(logs, 'tis_' + str(ts))
os.mkdir(log_dir)
dbcmd91 = DbCmd('172.31.240.82', '1521', 'AST91', 'system', 'enterprise', log_dir)
dbcmd91_obe = DbCmd('172.31.240.82', '1521', '99_NEW_PROD_ASSET_91', 'system', 'enterprise', log_dir)
...
# Importing regional data to project REFERENCE
...
# Importing files to satisfaction; in fact the server name is metallah.lan.mobistar.be
...
satisfaction_ssh = SSH(
    hostname = 'EDITED', username = 'EDITED', pkey = pk_satisfaction
)
...
freetown_ssh = SSH(hostname = 'EDITED', username = 'taskexec', pkey = freetown_pk)
freetown_ssh.sftp()
